...
EhsRouter::process_link_report(rtrmessage::bpa* msg)
{
    ...
    while (iter != c.end()) {
        rtrmessage::link_report::link::type& xlink = *iter;
...
//dz - add this?            el->set_throttle_bps(bps);
            ...
            log_msg(oasys::LOG_INFO, "process_link_report - Added Link: %s [%s]  initial state: %s", 
                     link_id.c_str(), el->remote_addr().c_str(), el->state().c_str());
...
EhsRouter::process_link_opened_event(rtrmessage::bpa* msg)
{
    ...
    EhsLink* el = NULL;
    if (find_iter == all_links_.end()) {
        rtrmessage::link_opened_event& xlink = msg->link_opened_event().get();
    ...
    if (el->is_rejected()) {
        log_msg(oasys::LOG_WARN, "process_link_opened_event: Rejecting Link %s from %s", 
                link_id.c_str(), el->remote_addr().c_str());
    } else {
        log_msg(oasys::LOG_INFO, "process_link_opened_event: Accepted Link (%s) from %s", 
                link_id.c_str(), el->remote_addr().c_str());
    }
...
EhsRouter::handle_bpa_received(EhsBpaReceivedEvent* event)
{
    ...
    if (in_bpa->link_report().present()) {
        ...
        log_msg(oasys::LOG_DEBUG, "received msg type: link_available_event");
        process_link_available_event(in_bpa);

    } else if (in_bpa->link_opened_event().present()) {
        log_msg(oasys::LOG_DEBUG, "received msg type: link_opened_event");
        process_link_opened_event(in_bpa);

    } else if (in_bpa->link_unavailable_event().present()) {
        log_msg(oasys::LOG_DEBUG, "received msg type: link_unavailable_event");
        process_link_unavailable_event(in_bpa);

    } else if (in_bpa->link_closed_event().present()) {
        log_msg(oasys::LOG_DEBUG, "received msg type: link_closed_event");
...
//----------------------------------------------------------------------
void
EhsRouter::send(rtrmessage::bpa& msg)
{
    parent_->send(msg);
}
