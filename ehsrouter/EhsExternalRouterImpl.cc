...
EhsExternalRouterImpl::configure_network_interface(std::string& val)
{
    ...
    if (0 == val.compare("any")) {
        ...
        if (ioctl(fd, SIOCGIFADDR, &ifr)) {
            ...
            local_addr_ = ((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr.s_addr;
        }
    } else {
...
{
    log_msg(oasys::LOG_INFO, "EhsExternalRouterImpl running on network_interface: %s", intoa(local_addr_));
...
//----------------------------------------------------------------------
void
EhsExternalRouterImpl::send(rtrmessage::bpa &message, const std::string* server_eid)
{
    oasys::ScopeLock l(&lock_, __FUNCTION__);
...
EhsExternalRouterImpl::process_action(const char *payload)
{
    ...
    std::auto_ptr<rtrmessage::bpa> instance;
    ...
    // all messages should include the EID of the DTN2 node
