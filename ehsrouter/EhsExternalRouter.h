...
public:
    ...
    /**
     * Issue shutdown request to the DTN2 server(s)
     */
   ...
   /**
     ...
     * between the DTN2 server and the EHS External Router.
    ...
    //
    virtual void bundle_stats(std::string& buf); // similar to the DTN2 bundle stats command output
    virtual void bundle_list(std::string& buf);  // list all bundles -- need to filter/limit/range number returned
    virtual void link_dump(std::string& buf);    // similar to the DTN2 link dump command output
