...
{
    // issue request to shutdown the DTN2 server
    rtrmessage::bpa msg;
...
EhsDtnNode::handle_bpa_received(EhsBpaReceivedEvent* event)
{
    ...
    if (in_bpa->link_report().present() ||
        ...
        log_msg(oasys::LOG_NOTICE, "received msg type: bundle_send_cancelled_event");
...
//----------------------------------------------------------------------
void
EhsDtnNode::send(rtrmessage::bpa& msg)
{
    msg.sequence_ctr(send_seq_ctr_++);
    parent_->send(msg, eid());
}
...
{
    return (0 == check_eid.compare(eid_) || 0 == check_eid.compare(eid_ipn_));
}
