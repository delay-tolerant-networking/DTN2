...
//----------------------------------------------------------------------
void
EhsLink::send(rtrmessage::bpa& msg)
{
    parent_->send(msg);
}
...
//dz - do not overwrite configured rate        rate_ = 0;
    ...
    if (0 == remote_eid_.compare(0, 4, "ipn:")) {
        char* endc;
        uint64_t nodeid = strtoull(remote_eid_.substr(4).c_str(), &endc, 10);
        if (NULL != endc && '.' == *endc) {
            source_nodes_[nodeid] = true;
        }
    }
...
//dz debug
parent_->log_msg(oasys::LOG_ALWAYS, "got null bundle from priority_tree_ - empty=%s size=%zu",
