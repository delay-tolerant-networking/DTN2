...
template <>
dtn::TestEhsExternalRouter* oasys::Singleton<dtn::TestEhsExternalRouter>::instance_ = 0;
...
TestEhsExternalRouter::output_header_provider_mode()
{
    ...
    if (!ehs_ext_router_->started()) {
        ...
        printf("    x = exit                              h = shutdown DTN2 server\n\n");
    }
...
TestEhsExternalRouter::bundle_stats_by_src_dst()
{
    ...
    for (int ix=0; ix<count; ++ix) {
        printf("%6"PRIu64" %6"PRIu64"  %8"PRIu64" %8"PRIu64" %8"PRIu64" %8"PRIu64" %8"PRIu64
               ...
               stats[ix].total_pending_, stats[ix].total_bytes_,
...
main(int argc, char** argv)
{
    ...
    printf("  2. Configure DTN2 with the  --with-ehsrouter   option and without the   --disable-edp   option\n");
    return 0;
}
