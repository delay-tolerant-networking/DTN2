...
BundleDaemon::handle_bundle_received(BundleReceivedEvent* event)
{
     ...
     */
    if (bundle->is_fragment_ && bundle->dest_.subsume(local_eid())) {
     ...
     */
    if ((event->source_ != EVENTSRC_ROUTER) && !bundle->is_fragment_) {
...
BundleDaemon::handle_bundle_inject(BundleInjectRequest* event)
{
    ...
    */
    ...
    // Deliver bundle to any local registrations; delivery of bundle
    // fragments is deferred until after re-assembly.
