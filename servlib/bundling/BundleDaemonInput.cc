...
template <>
dtn::BundleDaemonInput* oasys::Singleton<dtn::BundleDaemonInput, false>::instance_ = NULL;
...
bool BundleDaemonInput::shutting_down_ = false;
...
BundleDaemonInput::handle_bundle_received(BundleReceivedEvent* event)
{
    ...
    log_info("handle_bundle_received: validating "
    ...
    if (duplicate != NULL) {
        log_debug("got duplicate bundle: %s", bundle->gbofid_str().c_str());
...
BundleDaemonInput::run()
{
    ...
    while (1) {
        if (should_stop()) {
            log_debug("BundleDaemonInput: stopping - event queue size: %zu",
                      eventq_->size());
            break;
        }
