...
//----------------------------------------------------------------------
bool
Ciphersuite_C3::validate(const Bundle* bundle, BlockInfoVec*  block_list, BlockInfo* block,
...
//5. the actions must exactly reverse the transforming changes made in finalize()
    ...
    {  //yes - this is ours so go to work
		...
		log_debug_p(log, "Ciphersuite_C3::validate() locals->correlator() 0x%llx", locals->correlator());
		...
		while ( len > 0 ) {
			...
			switch ( item_type ) {
				case CS_IV_field: 
					{
						log_debug_p(log, "Ciphersuite_C3::validate() iv item, len = %llu", field_length);
				...
				case CS_C_block_salt:
					{
						log_debug_p(log, "Ciphersuite_C3::validate() salt item, len = %llu", field_length);
				...
				case CS_fragment_offset_and_length_field:
					{
						log_debug_p(log, "Ciphersuite_C3::validate() frag info item, len = %llu", field_length);
	         ...
	         ++iter)
	    {
	        ...
	        switch ( iter->type() ) {
	            ...
	            case BundleProtocol::CONFIDENTIALITY_BLOCK:
                {
					...
					while ( len > 0 ) {
						...
						switch ( item_type ) {
							case CS_IV_field: 
								{
									log_debug_p(log, "Ciphersuite_C3::validate() target iv item, len = %llu", field_length);
					...
					while ( len > 0 ) {
						...
						switch ( item_type ) {
							case CS_C_block_ICV_field: 
								{
									log_debug_p(log, "Ciphersuite_C3::validate() target icv item, len = %llu", field_length);
							...
							case CS_encap_block_field: 
								{
									log_debug_p(log, "Ciphersuite_C3::validate() encap block item, len = %llu", field_length);
                    ...
                    *ptr++ = *data++;				// block type
                    ...
                    data += sdnv_len;
                    len -= sdnv_len;
					log_debug_p(log, "Ciphersuite_C3::validate() target block type %hhu flags 0x%llx", *(preamble.buf()), flags);
                    ...
                    data += sdnv_len;
                    len -= sdnv_len;
					log_debug_p(log, "Ciphersuite_C3::validate() target data content size %llu", content_length);
                    ...
                    if ( eid_ref_count > 0 ) {
						...
						log_debug_p(log, "Ciphersuite_C3::validate() eid_ref_count %lld", eid_ref_count);
	            ...
	            case BundleProtocol::PAYLOAD_BLOCK: 
            	{
		            ...
		            iter->owner()->process( deliberate_const_cast_bundle,
                                            block,
                                            &*iter,
                                            Ciphersuite_C3::do_crypt,
                                            offset,
                                            len,
                                            r,
                                            changed);
                        ...
                        bool          	last)
{
    ...
    if ( need_correlator ) {
		...
		log_debug_p(log, "Ciphersuite_C3::generate() correlator %llu", correlator);
    }
...
log_debug_p(log, "Ciphersuite_C3::finalize() target_locals->correlator() 0x%llx", target_locals->correlator());
...
{    
    ...
    changed = (len > 0) ? true : false;
}

//----------------------------------------------------------------------
void
Ciphersuite_C3::init_block(BlockInfo* block, u_int8_t type, u_int8_t flags,
