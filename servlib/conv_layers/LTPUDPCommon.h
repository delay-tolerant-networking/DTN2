...
//dz #if defined(WORDS_BIGENDIAN) && (WORDS_BIGENDIAN == 1)
//dz #define htonq( x ) (x)
//dz #define ntohq( x ) (x)
//dz #else
//dz 
//dz inline u_int64_t htonq( u_int64_t x )
//dz {
//dz     u_int64_t   res;
//dz     u_int32_t   hi = x >> 32;
//dz     u_int32_t   lo = x & 0xffffffff;
//dz     hi = htonl( hi );
//dz     res = htonl( lo );
//dz     res = res << 32 | hi;
//dz 
...
//dz }
//dz 
//dz inline u_int64_t ntohq( u_int64_t x )
//dz {
//dz     u_int64_t   res;
//dz     u_int32_t   hi = x >> 32;
//dz     u_int32_t   lo = x & 0xffffffff;
...
public:
    ...
    int       Packet_Len()         { return packet_len_;        }
    int       Retrans_Retries()    { return retrans_retries_;   }


#ifdef LTPUDP_AUTH_ENABLED
    void      Create_Trailer();
    int       Parse_Trailer(u_char * bp, int current_byte, int length);
#endif
    ...
    int     Parse_Buffer(u_char * bp, int length, int * offset);

    static  LTPUDPSegment * DecodeBuffer(u_char * bp, size_t length);

public:

    class RetransmitTimer : public oasys::Timer,
                           public oasys::Logger 
    {
    public:
    ...
    void Start_Retransmission_Timer(void* timer);
...
protected:
    ...
    // security detail
    int        cipher_applicable_length_;
    int        cipher_trailer_offset_;
...
public:
    ...
    LTPUDPDataSegment(u_char * bp, int length);

    u_int32_t Offset()     { return offset_;     }       

    int Payload_Length () { return payload_length_; }

    int Start_Byte() { return start_byte_; }
    int Stop_Byte()  { return stop_byte_;  }
    ...
    void Encode_All();
    void Encode_Buffer();
    void Encode_Buffer(u_char * data, int length, int start_byte, bool checkpoint);

    int  Parse_Buffer(u_char * bp, int len);
    void Set_Endofblock();
...
public:
    ...
    LTPUDPReportClaim(int offset, int len);
...
public:
    ...
    LTPUDPReportSegment(u_char * bp, int len);
    ...
    u_int32_t  UpperBounds()           { return upper_bounds_;              }
    u_int32_t  LowerBounds()           { return lower_bounds_;              }
...
protected:
    ...
    u_int64_t report_serial_number_;
    u_int32_t upper_bounds_;
    u_int32_t lower_bounds_;
    u_int32_t claims_;
...
public:
    ...
    LTPUDPReportAckSegment(u_char * bp, int len);
...
protected:
    ...
    string key_; // hold the key for the timer... matches Report Key..
...
public:
    ...
    enum {
        LTPUDP_CS_REASON_USR_CANCLED,     // 0  Clien Service cancelled session
    ...
    LTPUDPCancelSegment(u_char * bp, int len, int segment_type);
    ...
    int  Parse_Buffer(u_char * bp, int len, int segment_type);

//dzdebug    char       ReasonCode()  { return reason_code_;  }
...
public:
    ...
    LTPUDPCancelAckSegment(u_char * bp, int len, int segment_type);
...
public:
    ...
    int        RemoveSegment  (DS_MAP * segments, int reception_offset, int reception_length);
    ...
    int                    Expected_Red_Bytes()     { return expected_red_bytes_;       }
    int                    Expected_Green_Bytes()   { return expected_green_bytes_;     }
    ...
    bool                   Has_Inactivity_Timer()   { return NULL != inactivity_timer_; }
...
protected:
    ...
    int  expected_red_bytes_;
    int  expected_green_bytes_;
    int  session_size_;
    int  reports_sent_;
