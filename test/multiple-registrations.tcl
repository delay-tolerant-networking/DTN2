...
test::script {
    ...
    puts "* Creating a handle on each node"
    set h0 [dtn::tell_dtntest 0 dtn_open]
    set h1 [dtn::tell_dtntest 1 dtn_open]

    puts "* Creating two passive registrations on node 0"
    ...
    puts "* Binding to the two registrations"
    dtn::tell_dtntest 0 dtn_bind $h0 $reg1
    dtn::tell_dtntest 0 dtn_bind $h0 $reg2

    puts "* Sending a bundle to each registration"
    ...
    puts "* Waiting for the bundles to arrive"
    dtn::wait_for_bundle_stats 0 "2 received"
    
    puts "* Receiving both bundles"
    ...
    puts "* Trying to receive something else, checking that it times out"
    ...
    puts "* Unbinding from one of the registrations"
    dtn::tell_dtntest 0 dtn_unbind $h0 $reg1

    puts "* Sending another bundle to both registrations"
    dtn::tell_dtntest 1 dtn_send $h1 \
            source=dtn://host-1/test \
            dest=dtn://host-0/test1 \
            payload_data="test"
    ...
    puts "* Receiving a bundle"
    set spec [dtn::tell_dtntest 0 dtn_recv $h0 payload_mem=true timeout=5000]

    puts "* Checking that the right one was delivered"
    foreach {var val} $spec {set bundle_data($var) $val}
    dtn::check_equal $bundle_data(dest) dtn://host-0/test2

    puts "* Checking that dtn_recv correctly times out"
    catch "dtn::tell_dtntest 0 dtn_recv $h0 payload_mem=true timeout=2000" recv_err

    puts "* Re-binding to the right registration"
    dtn::tell_dtntest 0 dtn_bind $h0 $reg1

    puts "* Checking that the bundle is now received properly"
    set spec [dtn::tell_dtntest 0 dtn_recv $h0 payload_mem=true timeout=5000]
    foreach {var val} $spec {set bundle_data($var) $val}
    dtn::check_equal $bundle_data(dest) dtn://host-0/test1

    puts "* Checking that many registrations works too"
